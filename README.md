...
6. 如果原站点还没有兼容线路域名，脚本会要求填写一个已解析到 VPS 的新域名。
7. 脚本显示客户端入口、兼容线路、API 上游、发现的推流上游和即将覆盖的站点；只有再次输入 `y` 才会备份、校验并覆盖。

普通站点以前没有配置任何推流域名也可以使用。发现推流地址后，先为推流线路填写兼容域名；确认覆盖后，脚本会把原普通反代升级为与菜单 3 相同的双域名“API 上游 + 推流上游”结构，自动生成兼容路径、内部推流路径、`Location` 改写和二次 302 防逃逸配置。客户端继续使用原入口域名、443 端口和空路径，新增域名用作兼容路径与备用直连。

由菜单 3 创建的双域名推流组重新检测时，会继续使用原兼容线路域名，并把新发现节点与已有节点合并后覆盖原组。此前由旧版菜单 6 生成的单域名“自动推流”站点也可以重新选择；重新检测成功后会要求填写兼容线路域名，并迁移为标准双域名推流组，因此站点列表最终会统一显示为 `[推流]`。
...
菜单 3 和菜单 6 都使用兼容线路域名。为保证默认入口、兼容线路域名的备用直连及其证书均可用，两条 DNS 记录都必须指向 VPS：
