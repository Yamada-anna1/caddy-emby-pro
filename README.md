...
- 播放失败时可临时捕获当前站点的 302，自动发现推流地址并经确认后升级原站点
...
当普通反代可以登录、浏览媒体库，但播放时失败，可以使用菜单 6：

1. 从 Caddyfile 中选择播放器当前正在使用的站点。
2. 脚本生成并加载临时运行配置，仅为所选域名记录安全目录中的 JSON access log；正式 Caddyfile 此时不变。
3. 在客户端播放到失败，也可以连续尝试多个视频，然后回到脚本按回车。
4. 脚本从绝对地址形式的 `302 Location` 中提取并去重实际推流源，最多支持 8 个节点。
5. 临时运行配置立即恢复，临时访问日志立即删除。
6. 脚本显示 API 上游、发现的推流上游和即将覆盖的站点；只有再次输入 `y` 才会备份、校验并覆盖。

普通站点以前没有配置任何推流域名也可以使用。确认覆盖后，脚本会把原普通反代升级为单入口的“API 上游 + 推流上游”结构，自动生成内部路径、`Location` 改写和二次 302 防逃逸配置；客户端地址、端口及空路径用法保持不变。由菜单 3 创建的双域名推流组也可以重新检测，新发现节点会与已有节点合并后覆盖原组。

自动提取 API 上游适用于本脚本生成的站点，以及站点块内只有一个明确 `reverse_proxy` 上游的旧配置。手工编写的复杂负载均衡、动态 matcher 或跨文件组合路由不会被猜测修改；无法可靠识别时脚本会中止并保留原配置。
...
- 临时配置无法验证或加载；
- 推流节点超过 8 个；
- 正式候选配置验证或 Caddy 重启失败。

临时日志可能在极短时间内包含播放请求参数，因此使用仅 `caddy` 用户和 root 可访问的目录，并在检测结束后删除。请勿中途强制结束 VPS 或杀死 Caddy；如按 `Ctrl+C` 取消，脚本会先尝试恢复正式运行配置。
